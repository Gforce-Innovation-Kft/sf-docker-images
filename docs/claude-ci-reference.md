...
- **The two package steps need `organization-packages: write`, not `packages: write`.** Pulling and pushing an image uses `packages`; setting a package's visibility (`PATCH /orgs/{org}/packages/...`) and deleting a version are org endpoints that 403 with `packages` alone. Both steps keep a `secrets.GITHUB_TOKEN` fallback and are `continue-on-error`, so a mis-scoped App degrades to today's behaviour instead of failing the gate.
