...
- **Base:** `ubuntu:22.04`. **Runtimes:** Node 20.x, OpenJDK 17 (JRE), SF CLI `@2.*`.
...
- **Base:** `ubuntu:22.04`. **Runtimes:** Node 20.x, OpenJDK 17 (JDK), SF CLI `@2.*`.
...
- **Base:** `node:20-alpine` + `coreutils` (needed for `env -S` in the SF CLI shebang on musl/BusyBox).
